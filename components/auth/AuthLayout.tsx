...
export default function AuthLayout({
  ...
  return (
    <div className="flex flex-col md:flex-row min-h-screen dark:bg-gray-900 transition-colors duration-200">
      {/* Left side - Auth form */}
      <motion.div
        className="w-full md:w-1/2 p-4 sm:p-8 flex items-center justify-center
                   bg-white dark:bg-gray-900 transition-colors duration-200 order-2 md:order-1"
        initial={{ opacity: 0, x: -20 }}
        animate={{ opacity: 1, x: 0 }}
        transition={{ duration: 0.5 }}
      >
        <div className="w-full max-w-md py-6 md:py-0">{children}</div>
      </motion.div>

      {/* Right side - Movie backdrop */}
      <motion.div
        className="relative w-full h-[40vh] md:h-auto md:w-1/2 md:block order-1 md:order-2"
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        {isLoading ? (
          ...
          <>
            ...
            <div className="absolute inset-0 overflow-hidden">
              {tmdbService.getBackdropURLProxy(featuredMovie.backdrop_path) && (
                <Image
                  src={
                    tmdbService.getBackdropURLProxy(
            ...
            {/* Movie details */}
            <motion.div
              className="absolute inset-0 flex flex-col justify-center px-6 sm:px-12 z-10"
            ...
            >
              <div className="max-w-sm">
                ...
                <motion.div
                  className="flex items-center space-x-3 mb-2 sm:mb-4"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                <motion.p
                  className="text-gray-700 dark:text-gray-300 text-sm sm:text-base line-clamp-2 sm:line-clamp-4 transition-colors duration-200"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                ...
                <motion.div
                  className="mt-3 sm:mt-6 hidden sm:block"
                  initial={{ opacity: 0, y: 10 }}
                  animate={{ opacity: 1, y: 0 }}
                  transition={{ duration: 0.3, delay: 1.6 }}
                >
                  <p className="text-gray-600 dark:text-white/70 text-sm transition-colors duration-200">
                    Join Filmguide to explore this and thousands of other
                    titles.
                  </p>
                </motion.div>
              </div>
          ...
          <div
            className="absolute inset-0 bg-gray-200 dark:bg-gray-900 flex items-center justify-center
                         transition-colors duration-200"
          >
