...
const CastMember = ({ person }: { person: Cast }) => {
  ...
  const profileUrl = person.profile_path ? tmdbService.getPosterURLProxy(person.profile_path, 'w185') : null;
  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        <h4 className="font-medium text-gray-900 dark:text-white truncate">{person.name}</h4>
        <p className="text-sm text-gray-600 dark:text-gray-400 truncate">{person.character}</p>
      </div>
    </motion.div>
  );
};
...
const ProductionInfo = ({ movie }: { movie: MovieDetails }) => (
  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-gray-200 dark:border-gray-800">
    {movie.production_companies && movie.production_companies.length > 0 && (
      <div>
        <h3 className="text-lg font-semibold mb-2 text-gray-800 dark:text-white">Production Companies</h3>
    ...
    {movie.production_countries && movie.production_countries.length > 0 && (
      <div>
        <h3 className="text-lg font-semibold mb-2 text-gray-800 dark:text-white">Production Countries</h3>
        <ul className="space-y-1 text-gray-700 dark:text-gray-300">
...
const LanguagesSection = ({ languages }: { languages: MovieDetails['spoken_languages'] }) => (
