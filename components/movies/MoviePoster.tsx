...
const ActionButtons = ({
  ...
  <div className="flex justify-between mb-6">
    ...
    >
      <svg
        className={`h-5 w-5 ${isHistory ? "text-blue-600" : ""}`}
        fill={isHistory ? "currentColor" : "none"}
        stroke="currentColor"
        viewBox="0 0 24 24"
      >
        <path
    ...
    >
      <svg
        className={`h-5 w-5 ${isWatchlist ? "text-red-600" : ""}`}
...
const MoviePoster = ({ movie }: { movie: MovieDetails }) => {
  ...
  const [userPrefInstance, setUserPrefInstance] =
  ...
  const posterUrl = movie?.poster_path
    ? tmdbService.getPosterURLProxy(movie.poster_path, "w500")
    : null;

  // Separate function to load user preferences
  const loadUserPreferences = useCallback(
    async (userPrefs: UserPreferences) => {
      if (!user) return;

      try {
        // Check if movie is in watchlist
        const watchlistStatus = await userPrefs.isInWatchlist(
          movie.id,
          user.id,
        );
