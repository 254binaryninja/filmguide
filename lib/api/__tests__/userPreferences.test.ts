...
describe('UserPreferences', () => {
  ...
  describe('getWatchlist', () => {
    ...
    it('should return empty array on error', async () => {
      const selectMock = vi.fn().mockResolvedValue({ data: null, error: new Error('Test error') });
      mockSupabase.from.mockReturnValue({ select: selectMock });
  ...
  describe('addToWatchlist', () => {
    it('should add movie to watchlist successfully', async () => {
      ...
      expect(mockSupabase.from).toHaveBeenCalledWith('watchlist');
      expect(insertMock).toHaveBeenCalledWith([{ movie_id: 1, user_id: 'user1' }]);
    });
  ...
  describe('removeFromWatchlist', () => {
    ...
    it('should throw error when removal fails', async () => {
      ...
      await expect(userPreferences.removeFromWatchlist(1, 'user1')).rejects.toThrow(
        'Failed to remove movie from watchlist: Test error'
      );
    });
  });
  ...
  describe('removeFromHistory', () => {
    ...
    it('should throw error when removal fails', async () => {
      ...
      await expect(userPreferences.removeFromHistory(1, 'user1')).rejects.toThrow(
        'Failed to remove movie from history: Test error'
      );
    });
  });
});